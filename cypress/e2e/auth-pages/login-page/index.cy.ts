...
describe('T1 - Login Page Test - Should be able to login using correct credentials', () => {
  beforeEach('Open the website home page', () => {
    ...
    cy.get(SELECTOR.LOGIN_PAGE.Login_Button_HP).click();
  });
  ...
  it('should not be able to log in via wrong email credentials', () => {
    ...
    cy.get(SELECTOR.LOGIN_PAGE.Login_Button).click();
    cy.get(SELECTOR.LOGIN_PAGE.Login_Button)
      .should('contain', 'Log in')
      .should('not.be.enabled');

    cy.get('#loading-4-msg').should('not.be.visible');
    cy.get(`${SELECTOR.LOGIN_PAGE.Email_Input_Text} .error-text`).should(
      'contain',
  ...
  it('should not be able to log in via five-digit password and should get an error message', () => {
    ...
    cy.get(SELECTORS.PAGES.GENERAL.Or_Text).click();
    // cy.get(SELECTOR.LOGIN_PAGE.Password_Input_Text);
    // cy.window().trigger('blur');
    // cy.get(SELECTOR.LOGIN_PAGE.Password_Input_Text).tab();
    // cy.get('.ion-justify-content-center').click();
    // cy.get(SELECTOR.LOGIN_PAGE.Login_Button).click();
    cy.get(SELECTOR.LOGIN_PAGE.Login_Button).should('not.be.enabled');
    // cy.get(`${SELECTOR.LOGIN_PAGE.Password_Input_Text} .error-text`).should('contain','password needs to be at least 8 digits long.');
    cy.get(`${SELECTOR.LOGIN_PAGE.Password_Input_Text} .error-text`).should(
      'be.visible'
    );
  });
  ...
  it('should not be able to log in via using a valid email, but the password should not contain special characters, and the password should be visible when clicking on the eye icon', () => {
    ...
    cy.get(SELECTOR.LOGIN_PAGE.Password_eye_icon).click();
    cy.get(`${SELECTOR.LOGIN_PAGE.Password_Input_Text}`).should(
      'have.attr',
      'type',
      'text'
    );
  ...
  it.only('After landing on the Login page, you should be able to connect the forget password button to see whether it goes to the  correct page.', () => {
    cy.get(SELECTOR.LOGIN_PAGE.Forget_Password_Btn).click();
